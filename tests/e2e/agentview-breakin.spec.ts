 ...
 * Run after bun run build, together with agentview.spec.ts using --workers=1:
 * the two specs deliberately share the fixture port 7793.
 */
...
const DAEMON = "http://127.0.0.1:7793";
...
async function startDaemon(flag: string) {
  daemon = await launch(
    ...
    {
      ...process.env,
      AGENTVIEW_E2E_DAEMON_PORT: "7793",
    },
  );
}
