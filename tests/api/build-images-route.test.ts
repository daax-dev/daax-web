...
const { mockCollectImages, mockRequireAuth, mockDockerUnavailable } =
  vi.hoisted(() => ({
    mockCollectImages: vi.fn(),
    mockRequireAuth: vi.fn(),
    mockDockerUnavailable: vi.fn(),
  }));

vi.mock("@/lib/build/images", () => ({ collectImages: mockCollectImages }));
vi.mock("@/lib/auth", () => ({ requireAuth: mockRequireAuth }));
vi.mock("@/lib/host-docker", () => ({
  getDocker: vi.fn(() => ({})),
  dockerUnavailableResponse: mockDockerUnavailable,
}));
...
describe("GET /api/build/images", () => {
  beforeEach(() => {
    ...
    mockDockerUnavailable.mockResolvedValue(null); // daemon reachable
  });
  ...
  it("returns 503 (not a list) when the Docker daemon is unreachable", async () => {
    mockDockerUnavailable.mockResolvedValue(
      NextResponse.json(
