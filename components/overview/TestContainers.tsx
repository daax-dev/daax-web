    ...
    <div className={cn("w-full max-w-5xl mx-auto", className)}>
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
          Stop mocking. Start testing against{" "}
          <span className="text-violet-400 font-semibold">
            real infrastructure
          </span>
          . TestContainers spins up actual databases, message queues, and cloud
          services as throwaway Docker containers— so your integration tests run
      ...
      <div className="p-6 rounded-xl bg-gradient-to-r from-violet-500/10 to-purple-500/10 border border-violet-500/20 mb-10">
        ...
        <p className="text-muted-foreground text-center max-w-2xl mx-auto">
          Mocks drift from reality. Shared test databases cause flaky tests.
          Local installs are painful to maintain. TestContainers solves this:{" "}
          <span className="text-foreground">
            each test gets a fresh, isolated, real service
          </span>
  ...
  it("creates and retrieves users", async () => {
    ...
    expect(found.name).toBe("Alice"); // Testing real SQL, not mocks
        ...
        <p className="text-sm text-muted-foreground mt-4">
          This test runs against a real PostgreSQL 16 instance. The container
          starts in ~2 seconds, runs your migrations, executes tests, then
          disappears. No cleanup scripts. No shared state.
        </p>
      </div>

      {/* Supported services */}
      <h3 className="font-semibold text-foreground mb-4 text-center">
        Spin Up Any Infrastructure
      </h3>
      ...
      <h3 className="font-semibold text-foreground mb-6 text-center">
        Why TestContainers Matter for AI Coding
      </h3>
      <div className="grid md:grid-cols-3 gap-6">
        <div className="p-5 rounded-xl bg-muted/10 border border-border/40">
          ...
          <p className="text-sm text-muted-foreground">
            When Claude Code writes a database migration, it can run the actual
            migration against a real PostgreSQL container and verify the schema
            is correct—not just check syntax.
          </p>
        </div>
        ...
        <div className="p-5 rounded-xl bg-muted/10 border border-border/40">
          ...
          <p className="text-sm text-muted-foreground">
            Containers start in seconds. AI agents can iterate rapidly—write
            code, test against real services, fix issues, test again—without
        ...
        <div className="p-5 rounded-xl bg-muted/10 border border-border/40">
          ...
          <p className="text-sm text-muted-foreground">
            Each test run is isolated. AI agents can&apos;t corrupt shared test
            data or leave behind state that breaks future runs. Containers are
            ephemeral by design.
          </p>
        </div>
      </div>
      ...
      <div className="mt-10 p-6 rounded-xl bg-muted/20 border border-border/50 text-center">
        <p className="text-muted-foreground">
          TestContainers works in CI too. GitHub Actions, GitLab CI, and Jenkins
          all support Docker-in-Docker. Your local tests and CI tests run
          against{" "}
