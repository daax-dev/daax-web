...
const sampleSBOM = {
  ...
  dependencies: [
    ...
    { name: "typescript", version: "5.7.3", license: "Apache-2.0", vulnerabilities: 0 },
    { name: "tailwindcss", version: "4.0.0", license: "MIT", vulnerabilities: 0 },
...
export function SBOMSample({ className }: { className?: string }) {
  ...
  const vulnCount = sampleSBOM.dependencies.reduce((sum, d) => sum + d.vulnerabilities, 0);
  ...
  return (
    <div className={cn("w-full max-w-4xl mx-auto", className)}>
      ...
      <div className="rounded-xl border border-border/50 bg-muted/10 overflow-hidden">
        ...
        <div className="p-4 border-b border-border/50 bg-muted/20">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-semibold text-foreground">{sampleSBOM.component.name}</span>
              <span className="text-muted-foreground ml-2">v{sampleSBOM.component.version}</span>
            </div>
        ...
        <div className="px-4 py-3 border-b border-border/30 flex items-center gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">{totalDeps} dependencies</span>
          </div>
          {vulnCount > 0 ? (
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-amber-400" />
              <span className="text-amber-400">{vulnCount} known vulnerability</span>
            </div>
          ) : (
            <div className="flex items-center gap-2">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {sampleSBOM.dependencies.map((dep) => (
                <tr key={dep.name} className="border-b border-border/20 hover:bg-muted/20">
        ...
        <div className="px-4 py-3 bg-muted/10 text-xs text-muted-foreground">
          Tools: {sampleSBOM.metadata.tools.join(", ")} • Formats: CycloneDX, SPDX
        </div>
      </div>
      ...
      <div className="mt-6 text-sm text-muted-foreground text-center">
        ...
        enabling vulnerability tracking, license compliance, and incident response.
      </div>
    </div>
  );
}
