...
const samplePermissions: Permission[] = [
  ...
  {
    name: "Execute npm/bun",
    description: "Run package managers",
    granted: true,
    category: "execution",
  },
  {
  ...
  },
  {
    ...
    description: "Access local dev servers",
    granted: true,
    category: "network",
  },
  {
    ...
    <div className={cn("w-full max-w-4xl mx-auto", className)}>
      ...
      <div className="text-center mb-10">
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          ...
          — precise guardrails with minimal friction. Explicit grants, explicit
          denials, no ambiguity.
        </p>
      </div>
      ...
      <div className="mt-6 flex items-start gap-3 p-4 rounded-lg bg-muted/20 border border-border/40">
        ...
        <p className="text-sm text-muted-foreground">
          Permissions are defined per-session or per-project. Agents operate
          within these bounds— any action outside the grant list is blocked and
          logged. Security without slowing you down.
        </p>
      </div>
    </div>
  );
