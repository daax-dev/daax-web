...
- **`terminal` (`daax-terminal` image)** — `server/terminal-server.ts` only (`start:terminal`, port 4201). Holds `docker.sock` (group-based access via `group_add: ${DOCKER_GID}`, non-root `node` user, boot preflight) + the workspace spawn machinery. It VERIFIES the WS ticket. Traefik's `/ws` route targets `127.0.0.1:4201`, now published by this service. Needs no Postgres and no Claude-config mount.
...
- **Supporting services (fleet, 2026-09-13):** the deploy compose also runs `code-server`, `watchtower`, `hawkeye` and `provenance`, each pulled by digest from its signed GHCR image (`CODE_SERVER_IMAGE`, `WATCHTOWER_IMAGE`, `HAWKEYE_IMAGE`, `PROVENANCE_IMAGE` in `deploy/env/<host>.env`; `*_OVERRIDE` digests win, as for `DAAX_IMAGE`). `scripts/deploy.sh --effective <target>` prints the effective pins (`DEPLOY_SH_API=2`) for dx `scripts/deploy-fleet.sh`.
- **Agent View on the fleet (2026-09-14):** the proxy can reach agentd over the tailnet at `AGENTVIEW_DAEMON_URL=https://agents.<host>.poley.dev` with a read-only `peer` bearer session (`AGENTVIEW_DAEMON_TOKEN_FILE`; host dir `AGENTVIEW_TOKEN_HOST_DIR` mounted at `/run/agentview`, minted and renewed by `deploy/host/agentview-token-renew.*`). No host relay, no extra network. Off loopback agentd withholds raw payloads (dist-agent ADR 0014) and refuses control to a peer session. Per-host rollout: `deploy/host/install-agentview-token.sh` → set the three variables in `deploy/env/<host>.env` together with a digest that carries this support → `scripts/deploy.sh <host>` → `deploy/host/retire-agentd-relay.sh` (verifies the tailnet path from inside daax before removing galway's legacy relay). The Console tab loads `agents.<host>` in the browser (`lib/agentview/console-url.ts`) and offers a top-level "Sign in to agentd", because Pocket ID refuses to be framed.
