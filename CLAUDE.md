...
- **`terminal` (`daax-terminal` image)** — `server/terminal-server.ts` only (`start:terminal`, port 4201). Sole holder of `docker.sock` (group-based access via `group_add: ${DOCKER_GID}`, non-root `node` user, boot preflight) + the workspace spawn machinery. It VERIFIES the WS ticket. Traefik's `/ws` route targets `127.0.0.1:4201`, now published by this service. Needs no Postgres and no Claude-config mount. **The terminal plane MUST stay single-instance (premortem R6):** WS-ticket single-use replay protection tracks used `jti`s in an in-memory `Map` (`server/handlers/ws-auth.ts`), so horizontally scaling / load-balancing the terminal service would let a ticket replay against a second instance. Move the replay set to shared state (Postgres/Redis) before scaling.
...
#### Authorization posture: single-operator by design (premortem A1)

daax-web has **flat authorization** — the `middleware.ts` default-deny gate *authenticates*, and only the admin-DB/provenance routes enforce a *role* (`requireRole`). Every other dangerous surface (mint a terminal ticket → PTY/`docker exec`, container control, secrets, MCP registry, clawd token) is `requireAuth`-only, and the baseline `user` role is granted `terminal:exec`/`containers:write`. **This is a deliberate single-operator posture:** any identity the Pocket ID IdP admits is treated as a full operator. It is safe for a single-operator install and NOT for a multi-tenant one — gate multi-user access at the IdP / Tailscale ACL layer (admit only trusted operators). Per-route `requireRole` + a restricted `user` baseline (privilege separation) is deferred; if adopted, it must fail closed. See `security-premortem-2026-08.md` §A1 and `.logs/decisions/security-premortem-2026-08.jsonl`.
