...
- **Agent View on the fleet (2026-09-14):** the proxy reaches agentd over the tailnet at `AGENTVIEW_DAEMON_URL=https://agents.<host>.poley.dev` with a read-only `peer` bearer session (`AGENTVIEW_DAEMON_TOKEN_FILE`, host dir `AGENTVIEW_TOKEN_HOST_DIR` mounted at `/run/agentview`, renewed by `deploy/host/agentview-token-renew.*`). No host relay, no extra network. Off loopback agentd withholds raw payloads (dist-agent ADR 0014) and refuses control to a peer session. The Console tab loads `agents.<host>` in the browser (`lib/agentview/console-url.ts`) and offers a top-level "Sign in to agentd", because Pocket ID refuses to be framed.
