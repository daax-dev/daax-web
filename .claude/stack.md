...
- Trust boundary: identity is derived entirely from the `X-Forwarded-*` headers; the app does not verify a token, so these headers are trusted as-is. `DAAX_REQUIRE_AUTH=1` only blocks the *absence* of a header — a client that can reach the app directly and set `X-Forwarded-User` is treated as that user. Therefore daax must be reachable **only** through the proxy, and the proxy must strip/overwrite any client-supplied `X-Forwarded-*` headers (Traefik forward-auth does this). Do not expose the app port directly on an untrusted network. Hardening the header trust (signed headers / proxy-IP allowlist) is tracked separately.
