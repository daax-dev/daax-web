...
- Package manager: **Bun `1.3.9`** — declared via `"packageManager": "bun@1.3.9"` in `package.json` (the production Dockerfile installs Bun from `bun.sh` without a version pin, so the container runtime version is not guaranteed to match). Use `bun install --frozen-lockfile`; `bun.lock` is committed. Do not introduce npm or yarn lockfiles. (Note: the production Dockerfile additionally installs `pnpm` only to fetch the global `backlog.md` CLI, and `npm` to rebuild the optional native `node-pty` — application dependencies are Bun-managed.)
