...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    const { initializeBacklogStore } = await import('@/server/backlog-multi-store');
    ...
    try {
      ...
      console.warn('[Instrumentation] Backlog features will be unavailable. Check /api/health/backlog for status.');
