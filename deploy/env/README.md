...
| `AGENTVIEW_TOKEN_HOST_DIR`             | optional; host directory bind-mounted read-only at `/run/agentview`. Must already exist, owned by the deploying user (preflight refuses otherwise). Install with `deploy/host/install-agentview-token.sh`                                  |
